    ...
    <div className="w-full bg-black fixed top-0">
        <div className="w-full max-w-screen-xl mx-auto bg-black text-white px-4 md:px-6 h-20 flex items-center justify justify-between">
            ...
            <ul className="hidden md:flex">
                {
                    myLinks.map(({link, id}) => (
                        <li key={id} className="capitalize text-xl font-medium px-4 text-gray-400 hover:scale-105 duration-200 ease-out cursor-pointer hover:text-gray-200">{link}</li>
                    ))
                }
            </ul>
